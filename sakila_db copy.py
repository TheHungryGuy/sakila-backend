...
def top_actors():
    ...
    ).join(FilmActor, Actor.actor_id == FilmActor.actor_id).group_by(Actor.actor_id, Actor.first_name, Actor.last_name).order_by(-func.count(FilmActor.film_id)).limit(5).all()
...
def top_movies_for_actor(actor_id):
    top_movies = db.session.query(
        ...
        Film.title,
        func.count(Rental.rental_id).label('rental_count')
    ).join(FilmActor, Film.film_id == FilmActor.film_id).join(Actor, FilmActor.actor_id == Actor.actor_id).join(Inventory, Film.film_id == Inventory.film_id).join(Rental, Inventory.inventory_id == Rental.inventory_id).filter(Actor.actor_id == actor_id).group_by(Film.film_id, Film.title).order_by(func.count(Rental.rental_id).desc()).limit(5).all()
...
def movie_copies_info():
    ...
    movie_copies_info = db.session.query(
        Film.film_id,
        Film.title.label('film_title'),
        func.count(Inventory.inventory_id).label('number_of_copies')
    ).outerjoin(Inventory, Film.film_id == Inventory.film_id).group_by(Film.film_id, Film.title).order_by(Film.film_id, func.count(Inventory.inventory_id).desc()).all()
...
def movie_info():
    ...
    if movie_id is None:
        ...
        results = db.session.query(
            Film.film_id,
            Film.title.label('film_title'),
            db.func.coalesce(db.func.count(Inventory.inventory_id), 0).label('number_of_copies'),
            db.func.coalesce(db.func.count(Rental.rental_id), 0).label('number_of_rentals_out'),
            db.func.coalesce(db.func.count(Inventory.inventory_id) - db.func.count(Rental.rental_id), 0).label('remaining_copies')
        ).outerjoin(Inventory, Film.film_id == Inventory.film_id).outerjoin(
            Rental, (Inventory.inventory_id == Rental.inventory_id) & (Rental.return_date.is_(None))
        ).group_by(Film.film_id, Film.title).order_by(Film.film_id).all()
    ...
    result = db.session.query(
        ...
        Rental, (Inventory.inventory_id == Rental.inventory_id) & (Rental.return_date.is_(None))
    ).filter(Film.film_id == movie_id).group_by(Film.film_id, Film.title).first()
